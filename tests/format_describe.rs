...
fn format_metar_contains_header_and_all_present_fields() {
    let metar =
        parse_metar("LIRF 121250Z 18010KT 9999 FEW030 -RA 18/12 Q1015 NOSIG").unwrap();
    ...
    assert!(text.contains("Visibility:"),  "expected visibility label: {}", text);
    assert!(text.contains("Weather:"),     "expected weather label: {}", text);
    assert!(text.contains("Clouds:"),      "expected clouds label: {}", text);
    assert!(text.contains("Temperature:"), "expected temperature label: {}", text);
    assert!(text.contains("Pressure:"),    "expected pressure label: {}", text);
...
fn format_metar_omits_absent_fields() {
    ...
    assert!(!text.contains("Weather:"),  "unexpected weather label: {}", text);
    assert!(!text.contains("Trend:"),    "unexpected trend label: {}", text);
    assert!(!text.contains("Remarks:"),  "unexpected remarks label: {}", text);
    assert!(!text.contains("Status:"),   "unexpected status label: {}", text);
}
...
fn format_metar_includes_remarks_when_present() {
    ...
    assert!(text.contains("Remarks:"), "expected remarks label: {}", text);
...
fn format_metar_includes_status_for_auto() {
    let metar =
        parse_metar("METAR LIRF 121250Z AUTO 18010KT 9999 FEW030 18/12 Q1015").unwrap();
    let text = format_metar(&metar, Language::En);
...
fn format_metar_multiple_weather_groups() {
    let metar =
        parse_metar("LIRF 121250Z 18010KT 9999 FEW030 -RA BCFG 18/12 Q1015").unwrap();
    let text = format_metar(&metar, Language::En);
...
fn format_metar_multiple_cloud_layers() {
    let metar =
        parse_metar("LIRF 121250Z 18010KT 9999 FEW015 SCT030 BKN080 18/12 Q1015").unwrap();
    let text = format_metar(&metar, Language::En);
...
fn format_metar_cavok_no_cloud_lines() {
    ...
    assert!(text.contains("Visibility:"), "expected visibility: {}", text);
    assert!(text.contains("CAVOK"),       "expected CAVOK text: {}", text);
    // CAVOK clears clouds in the parser, so no cloud lines expected
    assert!(!text.contains("Clouds:"), "unexpected cloud lines: {}", text);
}
...
fn format_taf_contains_header_and_base_forecast() {
    let taf =
        parse_taf("TAF LIRF 121100Z 1212/1318 18010KT 9999 SCT020").unwrap();
    ...
    assert!(text.contains("Validity:"),      "expected validity label: {}", text);
    assert!(text.contains("[Base forecast]"), "expected base block: {}", text);
    assert!(text.contains("Wind:"),          "expected wind label: {}", text);
    assert!(text.contains("Clouds:"),        "expected clouds label: {}", text);
}
...
fn format_taf_shows_tempo_block() {
    ...
    assert!(text.contains("[Temporary]"), "expected TEMPO block: {}", text);
...
fn format_taf_shows_becmg_block() {
    ...
    assert!(text.contains("[Becoming]"), "expected BECMG block: {}", text);
...
fn format_taf_shows_temperatures() {
    ...
    assert!(text.contains("Max temp:"), "expected max temp label: {}", text);
    assert!(text.contains("Min temp:"), "expected min temp label: {}", text);
}
...
fn format_taf_shows_wind_shear() {
    ...
    assert!(text.contains("Wind shear:"), "expected wind shear label: {}", text);
...
fn format_taf_amended_shows_status() {
    let taf =
        parse_taf("TAF AMD EDDF 181333Z 1813/1918 18005KT 9999 SCT025").unwrap();
    let text = format_taf(&taf, Language::En);
...
fn metar_description_display_via_to_string() {
    use metar_taf_parser::{describe_metar, Language};
...
fn taf_description_display_via_to_string() {
    use metar_taf_parser::{describe_taf, Language};
