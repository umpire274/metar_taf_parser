...
fn parse_single_color_code_grn() {
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 GRN",
    )
    .unwrap();

    assert_eq!(
        m.color_code,
        Some(MilitaryColor { code: MilitaryColorCode::Grn, black: false })
    );
...
fn parse_all_plain_color_codes() {
    for (token, expected) in [
        ...
        let input = format!("METAR EGVN 120930Z 25010KT 9999 FEW020 15/10 Q1013 {}", token);
...
fn parse_black_color_code() {
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 BLACKGRN",
    )
    .unwrap();
...
fn parse_black_amb_color_code() {
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 1200 OVC005 15/10 Q1013 BLACKAMB",
    )
    .unwrap();
...
fn parse_two_color_codes_current_and_forecast() {
    ...
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 9999 FEW020 15/10 Q1013 GRN BLU",
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some(MilitaryColor { code: MilitaryColorCode::Blu, black: false })
    );
}
...
fn parse_current_and_forecast_with_tempo() {
    ...
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 GRN AMB TEMPO RED",
    )
    .unwrap();
    ...
    assert_eq!(
        m.color_code_forecast,
        Some(MilitaryColor { code: MilitaryColorCode::Amb, black: false })
    );
    assert!(matches!(m.trend, Some(MetarTrend::Tempo)));
    let detail = m.trend_detail.unwrap();
    assert_eq!(
        detail.color_code,
        Some(MilitaryColor { code: MilitaryColorCode::Red, black: false })
    );
}
...
fn parse_color_code_in_becmg_trend() {
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 GRN BECMG BLU",
    )
    .unwrap();
...
fn parse_color_code_in_tempo_trend() {
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 GRN TEMPO AMB",
    )
    .unwrap();
...
fn parse_black_color_code_in_tempo_trend() {
    ...
    let m = parse_metar(
        "METAR EGVN 120930Z 25010KT 7000 BKN020 15/10 Q1013 GRN TEMPO BLACKRED",
    )
    .unwrap();
...
fn describe_color_code_grn() {
    let m =
        parse_metar("METAR EGVN 120930Z 25010KT 9999 FEW020 15/10 Q1013 GRN").unwrap();
...
fn describe_black_color_code() {
    let m =
        parse_metar("METAR EGVN 120930Z 25010KT 9999 FEW020 15/10 Q1013 BLACKGRN").unwrap();
    let desc = describe_metar(&m, Language::En);
    let cc = desc.color_code.unwrap();
...
fn describe_color_code_forecast() {
    ...
    assert!(forecast.contains("BLU") || forecast.contains("Blue"), "{}", forecast);
}
...
fn describe_color_code_in_tempo_trend() {
    ...
    assert!(trend.contains("AMB") || trend.to_lowercase().contains("amber"), "{}", trend);
}
